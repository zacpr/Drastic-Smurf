...
async fn main() -> eframe::Result<()> {
    ...
    let options = {
        let wgpu_setup = eframe::egui_wgpu::WgpuSetup::CreateNew(eframe::egui_wgpu::WgpuSetupCreateNew {
            power_preference: eframe::wgpu::PowerPreference::LowPower,
            instance_descriptor: eframe::wgpu::InstanceDescriptor {
                backends: eframe::wgpu::Backends::all(),
                flags: eframe::wgpu::InstanceFlags::default() | eframe::wgpu::InstanceFlags::ALLOW_UNDERLYING_NONCOMPLIANT_ADAPTER,
                ..Default::default()
            },
            native_adapter_selector: Some(std::sync::Arc::new(|adapters, _surface| {
                ...
                for (i, adapter) in adapters.iter().enumerate() {
                    ...
                    let msg = format!(
                        ...
                        i, info.name, info.backend, info.device_type, info.driver, info.driver_info
                    );
                    tracing::info!("{}", msg);
                    println!("{}", msg);
                }
                ...
                if let Some(adapter) = adapters.iter().find(|a| a.get_info().device_type == eframe::wgpu::DeviceType::DiscreteGpu) {
                    let info = adapter.get_info();
                ...
                if let Some(adapter) = adapters.iter().find(|a| a.get_info().device_type == eframe::wgpu::DeviceType::IntegratedGpu) {
                    let info = adapter.get_info();
