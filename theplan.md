...
## Additional asks from the same 2026-07-12 conversation, not yet started

These came up after the priority list above was written and should be slotted in — none has code written yet, but the first one below has API research already done:

- **System tray / minimize-to-tray for the Slint app.** Confirmed: the egui app already does this (`app.rs`'s `load_tray_icon()`, a plain UI-agnostic function building a `tray_icon::TrayIcon` + Show/Quit menu — directly reusable, just needs to become `pub`), but the Slint app currently has *no* tray integration at all — closing the window just closes it. User explicitly asked for this to be added, reusing the existing tray setup.

  Slint API confirmed via source read (`i-slint-core-1.17.1/api.rs`): `slint::Window::on_close_requested(callback: impl FnMut() -> CloseRequestResponse)`, where `CloseRequestResponse` has two variants — `HideWindow` (hides but keeps the event loop running; this is the `#[default]`) and `KeepWindowShown` (rejects the close). There's no "real quit" variant here — actually terminating requires calling `slint::quit_event_loop()` (from `i_slint_core::api`, re-exported at `slint::quit_event_loop`) separately, e.g. from the tray menu's "Quit" handler, not through the close-requested callback at all. This is actually simpler than the egui version's approach (which fakes a cancel-then-close via `ViewportCommand`).

  Plan: in `ui_slint::run()`, after `AppShell::new()`, call the (now-`pub`) `load_tray_icon()`, keep the returned `TrayIcon` alive for the process lifetime (e.g. `std::mem::forget` or held in a variable that outlives `app.run()`), set `app.window().on_close_requested(|| slint::CloseRequestResponse::HideWindow)`, and add a repeating `slint::Timer` (matching the pattern already used for async fetches elsewhere in `ui_slint/*.rs`) polling `tray_icon::TrayIconEvent::receiver().try_recv()` (left-click → `app.show()`) and `tray_icon::menu::MenuEvent::receiver().try_recv()` (Show-item click → `app.show()`; Quit-item click → `slint::quit_event_loop()`).

- **Restore Dashboard to "former glory and then some."** Current Slint Dashboard (`ui/dashboard.slint`) only has the Overview grid mode (`DashboardOverview`) — the "Detailed single cluster" mode from this doc's own "Agreed redesign direction" section above (header card with API/Kibana/HAProxy link chips, four metric cards, node list table, shard allocation/disk table) was never built; it got scoped out when Dashboard was first built. User wants it back, "and then some" (some amount of enhancement beyond the original spec — not yet clarified exactly what). Note there's already significant overlap with what `ui/status.slint` built later in this session (header card + 4 metric cards + node table with CPU/RAM/heap UsageBars + shard allocation table) — worth reusing/adapting those patterns/components rather than rebuilding from scratch. Needs a quick scope-clarifying question at the start of the next session on what "and then some" should include.

- **Quickstart/onboarding wizard for the zero-clusters empty state.** This resolves the "empty-state / quickstart flow... not designed" open question above — it turns out it *is* designed, and already built in the egui app: `src/ui/wizard.rs` (539 lines), an 11-step modal tour (`WizardStep` enum: Welcome → AddCluster (a real inline add-cluster form) → Dashboard → ClusterStatus → SnapshotMon → TaskMon → Indices → Observability → ElasticConsole → DiscoverTab → WorldClocks → Finish), driven by `WizardState` and shown via `render_wizard_overlay`. Triggered today by `config.wizard_completed` being false on first launch. This is a port, not a new design — same pattern as Painless/AI Assistant above.

- **AI Assistant voice interaction** (musing, not yet agreed to implement) — user asked about feasibility of talking to / having the assistant talk to the user. Recommended splitting into two independent pieces when the AI Assistant port (`modules/llm_assistant.rs`) is tackled: text-to-speech output (easier — wrap a platform or cloud TTS API around the already-streaming response text) and speech-to-text input (harder — needs mic capture, a recognition backend, and push-to-talk UX that doesn't exist anywhere in the app yet). Treat as a follow-on enhancement to the AI Assistant port, not a blocker for it.
